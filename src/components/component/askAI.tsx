...
const AskAI = ({ question }: Props) => {
  ...
  const handleAsk = async () => {
    ...
    const prompt = `Generate answer with explanation for given question.
  ...
  return (
    <div className="relative">
      ...
      {loading || aiResponse ? (
        <div
          ref={responseRef}
          className="bg-white rounded-md border max-h-96 overflow-auto  p-3 right-0 w-[25vw]  shadow-lg z-50 absolute top-full translate-y-2"
        >
          {loading ? (
            <div className="animate-loading rounded-full h-3 w-3 bg-black"></div>
          ) : (
            <div className="flex flex-col gap-2">
              <p className=" w-full  max-h-96 whitespace-pre-wrap ">
                {aiResponse}
              </p>
